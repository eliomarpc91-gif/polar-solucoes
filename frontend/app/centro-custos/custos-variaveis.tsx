...
import { getCentroDeCustos, adicionarCustoVariavel, removerCustoVariavel } from "@/lib/hh-store";
import { CustoVariavel } from "@/lib/hh-types";
...
const CATEGORIAS = [
  ...
  { label: "Desgaste de Ferramenta", value: "desgaste_ferramenta" },
  { label: "Garantia", value: "garantia" },
  { label: "Retorno Técnico", value: "retorno_tecnico" },
  { label: "Imprevistos", value: "imprevistos" },
  { label: "Outros", value: "outros" },
];

export default function CustoVariavelScreen() {
  ...
  const carregarCustos = async () => {
    ...
    if (centro) {
      setCustos(centro.custos_variaveis);
    }
  };
  ...
  const salvarCusto = async () => {
    ...
    const novoCusto = await adicionarCustoVariavel({
    ...
    Alert.alert("Sucesso", "Custo variável adicionado!");
  };
    ...
    <ScreenContainer className="px-5 pt-4">
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 30 }}>
        <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          ...
          <Text className="text-foreground text-2xl font-bold">Custos Variáveis</Text>
          <Pressable
        ...
        {adicionando && (
          <View style={{ backgroundColor: colors.surface, borderRadius: 14, padding: 16, marginBottom: 20, borderWidth: 1, borderColor: colors.border }}>
            <Text className="text-foreground font-bold mb-4">Novo Custo Variável</Text>
            ...
            <TextInput
              ...
              placeholder="Ex: Combustível - Deslocamento SP"
              style={inputStyle}
            />
        ...
        {custos.length === 0 ? (
          <View style={{ alignItems: "center", justifyContent: "center", paddingVertical: 40 }}>
            ...
            <Text className="text-muted text-sm mt-3">Nenhum custo variável cadastrado</Text>
          </View>
        ) : (
        ...
        {custos.length > 0 && (
          <View style={{ backgroundColor: colors.surface, borderRadius: 14, padding: 16, marginTop: 20, borderWidth: 1, borderColor: colors.border }}>
            <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
              <Text className="text-muted text-sm">Total do Mês:</Text>
